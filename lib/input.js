...
function readInt(radix, len) {
    ...
    for (var i = 0, e = len == null ? Infinity : len; i < e; ++i) {
        var code = input.charCodeAt(tokPos), intval;
        if (code >= 97)
            intval = code - 97 + 10; // a
        else if (code >= 65)
            intval = code - 65 + 10; // A
...
function readNumber(startsWithDot) {
    ...
    var str = input.slice(start, tokPos), intval;
    if (isFloat)
...
function parseForIn(node, init) {
    var forintype = tokType === _in ? "ForInStatement" : "ForOfStatement";
    next();
    node.left = init;
