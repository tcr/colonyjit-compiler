...
typedacorn.compile(fs.readFileSync(process.argv[2]), function (err, data) {
  ...
  var funcTypes = {
    ...
    parseExprOp: ['Node*', 'Node*', 'int', 'bool'],
    ...
    parseObj: ['Node*'],
    parseNew: ['Node*'],
    parsePropertyName: ['int', 'Node*'],
    parseFunctionParams: ['int', 'Node*'],
    parseFunctionBody: ['int', 'Node*', 'bool'],
    parseStatement: ['Node*'],
    parseTemplate: ['Node*'],
    parseSpread: ['Node*'],
    ...
    if (node.type == 'FunctionExpression' || node.type == 'FunctionDeclaration') {
      ...
      if (node.id) {
        ...
        if (type && funcTypes[id]) {
          ...
          if (fntype.join(' ') == funcTypes[id].join(' ')) {
            // console.error(id);
          } else {
            console.error('invalid:', id, fntype, funcTypes[id]);
          }
        } else if (!type) {
          // console.error('---> no type', id, getType(node));
        }
        // if (id == 'getLineInfo') {
        //   console.error(node.loc, data.types.filter(function (k) {
        //     return k.kind == 'function'
        //   }));
        // }
      }
    }
