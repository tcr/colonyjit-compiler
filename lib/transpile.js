...
// Rework character class regexes.
{
  function makeregexfn (line) {
    if (line.match(/var/)) {
      ...
      return '/*C ' + makeregex(name, chars, 'return true;', 'break;', 'return false;') + '*/';
    }
    return line;
  }
  ...
  infuse(448, function (line) {
    var name = line.match(/var (\S+)/)[1];
    return '/*C ' + makeregex(name, regexcache.nonASCIIidentifierStartChars, 'return true;', 'break;', 'return false;') + '*/';
  })

  infuse(449, function (line) {
    var name = line.match(/var (\S+)/)[1];
    return '/*C ' + makeregex(name, regexcache.nonASCIIidentifierStartChars + regexcache.nonASCIIidentifierChars, 'return true;', 'break;', 'return false;') + '*/';
  })
