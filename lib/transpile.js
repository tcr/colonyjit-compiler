...
// wipe(132, 144);        // removes "getLineInfo"
...
// rework regexes into a typed lambda
// rework makePredicate into a macro
// rework keywordTypes array into something?

var hoistedregex = [];

// Rework character class regexes.
// {
//   function makeregexfn (line) {
//     if (line.match(/var/)) {
//       var chars = JSON.parse('"' + line.replace(/^.*?\/\[|\]\/.*?$/g, '').replace(/"/g, '\\"').replace(/\\x(..)/g, '\\u00$1') + '"');
//       var name = line.match(/var (\S+)/)[1];
//       hoistedregex.push(makeregex('regex_' + name, chars, 'return true;', 'break;', 'return false;'))
//       return 'var ' + name + ' = ' + 'regex_' + name + ';';
//     }
//     return line;
//   }

//   infuse(447, makeregexfn);
//   infuse(448, makeregexfn);
//   infuse(449, makeregexfn);
//   infuse(453, makeregexfn);
//   infuse(458, makeregexfn);
// }

// Replace "makePredicate" optimization 
replace(/var (\w+)\s*=\s*makePredicate\((.*?)\);/g, function (_, name, args) {
  args = args.replace(/"|^\s*|\s*$/g, '').split(/\s+/);
  var cases = args.map(function (str) {
    return 'arg == ' + JSON.stringify(str);
  });
  return '/*C bool ' + name + '(std::string arg) { return ' + cases.join(' || ') + '; } */';
})
