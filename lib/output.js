...
raise(tokPos - 2, "Unterminated comment");
    ...
    //     options.onComment(true, input.slice(start + 2, end), start, tokPos, startLoc, options.locations && Position);
}
...
auto skipLineComment() {
    ...
    // if (options.onComment)
    //     options.onComment(false, input.slice(start + 2, tokPos), start, tokPos, startLoc, options.locations && Position);
}
...
Node* parseTopLevel(Node* program) {
    ...
    // if (options.locations)
    //     lastEndLoc = Position;
    inFunction = inGenerator = strict = 0;
...
Node* parseForIn(Node* node, Node* init) {
    auto forintype = tokType==_in ? "ForInStatement" : "ForOfStatement"; 
    next();
    node->left = init;
    ...
    node->body = parseStatement();
    pop(labels);
    return finishNode(node, forintype);
}
    ...
    if (tokType.prefix) {
{
        ...
        if (update) {
5;
} else if (strict && node->opr=="delete" && node->argument->type=="Identifier") {
...
Node* parseObj() {
    auto node = startNode();  auto first = true; //, propHash = {};
