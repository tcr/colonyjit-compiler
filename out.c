    ...
    auto line = 1;  auto cur = 0; ; for (; ;)
{
  ...
  auto inFunction = 0;  auto labels = 0;  auto strict = 0; 
  ...
  auto nonASCIIidentifierStart = RegExp("[" + nonASCIIidentifierStartChars + "]"); 
  auto nonASCIIidentifier = RegExp("[" + nonASCIIidentifierStartChars + nonASCIIidentifierChars + "]"); 
    ...
    ; for (; ;)
{
  ...
  auto finishNode(auto node, keyword_t type) {
    node->type = type;
    node->end = lastEnd;
    if (options.locations)
